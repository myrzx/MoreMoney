...
| Renderer | `src/*` | 薪资计算、UI 渲染、动画、设置表单、日历 |
...
- **工作日判断**：dayOverrides > holidays.json > 默认周一到周五（硬编码）
...
日历视图展示每日工时，支持手动打卡和工作日切换：

- **日历渲染**：按月展示，RPG 品质配色（普通灰蓝/优秀绿/史诗紫/传说金），双层像素边框
- **手动打卡**：点击工作日格子弹出时间选择器，保存上下班时间
- **右键切换**：右键任意格子可标记为工作日/休息日，或恢复默认（优先级：dayOverrides > holidays.json > 默认周一到周五）
- **自动记录**：`autoRecord()` 每 60s 检查，自动为当天和过去的日子创建/补全记录
- **工时计算**：扣除休息时间，超过 8h 红色标注，不足 8h 蓝色标注
- **月度统计**：月标题下方单行显示（总工时 − 工作天×8 = 奉献工时，日均工时）
- **持久化**：`userData/records.json` 存储每日打卡记录 + dayOverrides
...
| 内联样式 | 设置页样式写在 `<style>` 标签内，可考虑抽到共享 CSS |
