...
**Main process** (`main.js`): Window management (3 frameless BrowserWindows), system tray, 11 IPC handlers, config/records file I/O, auto-record timer, clock-out reminder scheduler.

**Preload** (`preload.js`): contextBridge exposes `window.electronAPI` with 11 promise-based IPC methods (all invoke/handle, no send/on).
...
- Closing main window hides to tray (does not quit); settings/calendar windows close normally
